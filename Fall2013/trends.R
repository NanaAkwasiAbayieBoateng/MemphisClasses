	...
	for( i in 1:nStrata ){
		...
		for(j in 1:length(p)){
			if( sum(strata.fit$surv<=p[j]) ){
				...
				rslt[[i]][j,3] <- ifelse( sum(strata.fit$lower>p[j]) >= 1, strata.fit$time[ max(which(strata.fit$lower>p[j]))+1 ], 0 )
				rslt[[i]][j,4] <- ifelse( sum(strata.fit$upper<p[j]) >= 1, strata.fit$time[ min(which(strata.fit$upper<p[j])) ], Inf )
				}
			}
		}
	return(rslt)
...
survtrend <- function( formula, data, print.table=TRUE ){
	...
	cat( "\nTarone Test Trend : Chi(1) = ", score, ", p-value = ", 1-pchisq(score, 1), "\n", sep="" )
	}
	

##
#####	plotKM() : Function to plot KM estimates that allows for labeling
...
##
plotKM <- function( fit, xscale=365.25, xlab="Time from study start (years)", ylab="Survival Probability", lty=1, 
...
##
glmCI <- function( model, transform=TRUE ){
	link <- model$family$link
	coef <- summary( model )$coef
	if( is.element(link, c("logit","log","cloglog")) & transform ){
	...
	rslt <- round( cbind( est, ci95.lo, ci95.hi, coef[,3:4] ), 4 )
	colnames( rslt )[1] <- ifelse( is.element(link, c("logit","log","cloglog")) & transform, "exp( Est )", "Est" )
	rslt
	}
